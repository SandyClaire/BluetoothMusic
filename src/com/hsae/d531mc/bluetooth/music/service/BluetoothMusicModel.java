...
public class BluetoothMusicModel {
    ...
    private static final int AUDIO_FOCESED = 1;
    private static final int AUDIO_LOSED = 0;

    public static final int REASON_OUT_ = 1;
    public static final int REASON_IN_ = 2;
    public static final int REASON_AUDIOFOCUS_CHANGED = 3;
    ...
    private boolean isNotifyLauncher = true;

    public boolean isCanPositionNotify = false;
    ...
    public MusicCallback mMusicProxyCallback = new MusicCallback() {
        ...
        public void onPlayStatusChanged(int state) {
            ...
            BluetoothMusicModel.this.onPlayStatusChanged(state, playReason);
    ...
    private static final int IS_SEND_POSITION = 5;
    ...
    public void requestAudioFocus(final boolean showOrBack, boolean fromPlay) {
        LogUtil.i(TAG, "cruze  BT getCurrentSource = " + mSource.getCurrentSource() + ",isHandPuse = " + isHandPuse
                + ", fromPlay =" + fromPlay);
        try {
            if (fromPlay) {
                doPlay(showOrBack);
                ...
                // }
                if (isAudioFocused) {
                    isNotifyLauncher = false;
                } else {
                    isNotifyLauncher = true;
                }
            ...
            case AudioManager.AUDIOFOCUS_LOSS:
                ...
                try {
                    if (callstatus == 0 || callstatus == -1) {
                        ...
                        isPhone = (!isPlaying);
                        LogUtil.i(TAG, "audiofocus loss caused by callstatus , pause by mobile self" + isPlaying);
                    }
    ...
    public synchronized void notifyAutroMusicInfo(MusicBean bean, boolean fromStream, boolean fromPoweroff) {
        ...
        if (fromStream) {
            Log.i(TAG, "notifyAutroMusicInfo hasSet = " + hasSet + ", isAudioFocused = " + isAudioFocused
                    + " , isSyncPostionToMcan = " + isSyncPostionToMcan);
            if (isAudioFocused) {
                if (!hasSet && isSyncPostionToMcan) {
                    BTMusicInfo info = new BTMusicInfo(title, atrist, album, null);
                    syncMusicInfo(info, false);
                } else {
                    hasSet = false;
                }
    ...
    private void syncMusicInfo(BTMusicInfo info, boolean isId3) {
        ...
        if (isId3) {
            ...
            if (!isCanPositionNotify && (!handler.hasMessages(IS_SEND_POSITION))) {
                handler.sendEmptyMessageDelayed(IS_SEND_POSITION, 1500);
            }
        }
    ...
    private void notifyLauncherInfo() {
        ...
        if (connStatus == MangerConstant.Anw_SUCCESS && mBean != null) {
            LogUtil.i(TAG, "notifyAutroMusicInfo,notifyLauncherInfo");
            hasSet = false;
    ...
    public void setStreamMute() {
        try {
            ...
            if (!isAudioFocused || isDiagnoseMode) {
                audioSetStreamMode(MangerConstant.AUDIO_STREAM_MODE_DISABLE);
            } else if (isAudioFocused && !isDiagnoseMode) {
                audioSetStreamMode(MangerConstant.AUDIO_STREAM_MODE_ENABLE);
            }
    ...
    public BTMusicInfo getCurrentMusicInfo() {
        return new BTMusicInfo(lastTitle, lastAtrist, lastAlbum, null);
    }
    ...
    public synchronized void updateConnectState() {
        if (mBluetoothAllCallback != null) {
            ...
            mBluetoothAllCallback
                    .onConnectStateChanged(MangerConstant.PROFILE_AUDIO_STREAM_CHANNEL, getA2dpStatus(), 0);
            mBluetoothAllCallback.onConnectStateChanged(MangerConstant.PROFILE_AUDIO_CONTROL_CHANNEL, getAvrcpStatus(),
                    0);
        }
    }
    ...
    public void onFocusChanged(int focus) {
        synchronized (lockOfBTMmanager) {
            try {
                int n = mBTMmanager.mListeners.beginBroadcast();
                for (int i = 0; i < n; i++) {
                    mBTMmanager.mListeners.getBroadcastItem(i).onFocusChange(focus);
                }
                mBTMmanager.mListeners.finishBroadcast();
            } catch (Exception e) {
                LogUtil.i(TAG, " ---- Exception = " + e.toString(), e);
    ...
    public void appShow(boolean onAppShow) {
        synchronized (lockOfBTMmanager) {
            try {
                int n = mBTMmanager.mListeners.beginBroadcast();
                for (int i = 0; i < n; i++) {
                    if (onAppShow) {
                        mBTMmanager.mListeners.getBroadcastItem(i).onAppShow();
                    } else {
                        mBTMmanager.mListeners.getBroadcastItem(i).onAppHide();
                    }
                }
                mBTMmanager.mListeners.finishBroadcast();
            } catch (Exception e) {
    ...
    int last = -1;
    public void onPlayStatusChanged(int state, int reason) {
        if (last !=state) {
            last = state;
            synchronized (lockOfBTMmanager) {
                try {
                    int n = mBTMmanager.mListeners.beginBroadcast();
                    for (int i = 0; i < n; i++) {
                        mBTMmanager.mListeners.getBroadcastItem(i).onPlayStatusChanged(state, reason);
                    }
                    mBTMmanager.mListeners.finishBroadcast();
                } catch (Exception e) {
                    LogUtil.i(TAG, " ---- Exception = " + e.toString(), e);
